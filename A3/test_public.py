...
def test_load_data(train_data):
    ...
    assert len(train_contents) == 14925
    assert (
        train_contents[0][0:120]
        == "Received: from nahou-mscnx06p.corp.enron.com ([192.168.110.237]) by "
        "NAHOU-MSMBX01V.corp.enron.com with Microsoft SMTPSVC"
    )
    assert (
        train_contents[0][-121:]
        == "F084-ÉãÓ°½²×ù £¤15.00 1¸ö¹âÅÌ ÊÓÆµ AF085-³ÂºÆ-¡¶ÌìÀÇÖ®ÑÛ¡· £¤30.00 "
        "2¸ö¹âÅÌ ÊÓÆµ BK010-µçÊÓÉãÏñÒÕÊõ £¤40.00 2¸ö¹âÅÌ ¶àÃ½Ìå"
    )

    assert len(train_labels) == 14925
    assert train_labels[55:65] == [1, 0, 1, 1, 1, 1, 1, 1, 0, 1]
...
def test_preprocess(train_data):
    ...
    assert "technologies" in preprocessed_example
...
def test_preprocess_multiple(processed_train_data, processed_test_data):
    ...
    assert len(processed_train_data) == 14925
    assert len(processed_test_data) == 1655


def test_extract_features(feature_vectors):
    """This test is 0.4 points."""
    train_vectors, test_vectors = feature_vectors
    assert train_vectors.shape[0] == 14925
    assert train_vectors.shape[1] > 0
    assert test_vectors.shape[0] == 1655
...
def test_train(classifier, feature_vectors):
    ...
    assert len(validation_prediction) == 1655
